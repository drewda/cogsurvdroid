...
public class CogSurvDroid extends Application {
  ...
  public boolean landmarksLoaded = false;
  ...
  public boolean isReady() {
    return getCogSurver().hasLoginAndPassword()
  ...
  /**
   * Provides static access to a Foursquare instance. This instance is initiated
   * without user credentials.
   * 
   * @param context
   *          the context to use when constructing the Foursquare instance
  ...
  public boolean recordTravelFix(TravelFix travelFix) {
    ...
    Log.d("CogSurv", "recordTravelFix: insert into ContentProvider: localId = "
        + localId);
      ...
      if (landmarks.size() > 0) {
        for (Landmark landmark : landmarks) {
          int rowsDeleted = getContentResolver().delete(
              LandmarksColumns.CONTENT_URI,
              LandmarksColumns.SERVER_ID + "=" + landmark.getServerId(), null);
          Log.v("CogSurv",
              "CogSurvDroid.readLandmarks(hitServer=true) rows deleted: "
                  + rowsDeleted);
          Uri uri = getContentResolver().insert(LandmarksColumns.CONTENT_URI,
              CogSurverProvider.createContentValues(landmark));
          Log.v("CogSurv",
              "CogSurvDroid.readLandmarks(hitServer=true) row inserted: "
                  + uri.toString());
        }
      }
    }
  ...
  public LandmarkVisit recordLandmarkVisit(LandmarkVisit landmarkVisit) {
    ...
    long localId = Long.parseLong(uri.getLastPathSegment());
    Log.d("CogSurv",
        "recordLandmarkVisit: insert into ContentProvider: localId = "
            + localId);
  ...
  public DirectionDistanceEstimate recordDirectionDistanceEstimate(
    ...
    Uri uri = getContentResolver().insert(
        DirectionDistanceEstimatesColumns.CONTENT_URI,
        CogSurverProvider.createContentValues(directionDistanceEstimate));
    long localId = Long.parseLong(uri.getLastPathSegment());
    Log.d("CogSurv",
        "recordDirectionDistanceEstimate: insert into ContentProvider: localId = "
            + localId);
  ...
  public Dialog showProgressDialog(String message, String title, Context context) {
